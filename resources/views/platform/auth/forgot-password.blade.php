...
<body class="login-bg min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
    ...
    <div class="relative w-full max-w-md fade-in">
        <div class="card-shine rounded-2xl border border-gray-200/80 shadow-xl shadow-gray-200/50 overflow-hidden">
            ...
            <div class="p-8 sm:p-10">
                <div class="text-center mb-8">
                    ...
                    <p class="text-gray-500 text-sm mt-1.5">No worries, we'll send you a reset link</p>
                </div>
                ...
                <form id="forgotForm" class="space-y-5">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="email" id="email" placeholder="admin@yoursaas.com" required autofocus
        ...
        <p class="text-center text-xs text-gray-400 mt-6">Remember your password? <a href="{{ route('platform.login') }}" class="text-blue-500 hover:text-blue-600 hover:underline">Sign in instead</a></p>
    </div>
    ...
    <script>
        document.getElementById('forgotForm').addEventListener('submit', function(e) {
            ...
            fetch('{{ route('platform.password.email') }}', {
                method: 'POST',
            ...
            .then(data => {
                ...
                if (data.token) box.textContent += ' (Dev: ' + data.token.substring(0, 30) + '...)';
                box.style.display = 'block';
            })
            .catch(() => { box.className = 'msg-box error'; box.textContent = 'Something went wrong.'; box.style.display = 'block'; })
            .finally(() => { btn.disabled = false; text.textContent = 'Send Reset Link'; spinner.style.display = 'none'; });
        });
    </script>
</body>
</html>